...
void setup() {
  // put your setup code here, to run once:
    ...
    status_return_level(ID1,1);
    status_return_level(ID2,1);
    status_return_level(ID3,1);
...
void loop() {
    ...
    sync_pos_move3(ID1,input1,ID2,input2,ID3,input3);
    ...
    read_present_vel(ID1);
    
    read_present_vel(ID2);
    
    read_present_vel(ID3);
}
...
int read_byte(int n){
  ...
  int index=1;
while(Serial3.available()){
    byte leng = Serial3.readBytes(data,n);
    _delay_us(250);
  Serial.println();
      if(index==15) break;
      index++;
